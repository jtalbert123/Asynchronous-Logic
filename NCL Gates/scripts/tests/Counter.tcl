...
proc set_inputs {A B C Control} {
  if {$A} {
    force -deposit /counter_tb/iA.DATA1 1 [expr round(rand()*5)]
	force -deposit /counter_tb/iA.DATA0 0 [expr round(rand()*5)]
  } else {
    force -deposit /counter_tb/iA.DATA1 0 [expr round(rand()*5)]
	force -deposit /counter_tb/iA.DATA0 1 [expr round(rand()*5)]
  }
  
  if {$B} {
    force -deposit /counter_tb/iB.DATA1 1 [expr round(rand()*5)]
	force -deposit /counter_tb/iB.DATA0 0 [expr round(rand()*5)]
  } else {
    force -deposit /counter_tb/iB.DATA1 0 [expr round(rand()*5)]
	force -deposit /counter_tb/iB.DATA0 1 [expr round(rand()*5)]
  }
  
  if {$C} {
    force -deposit /counter_tb/iC.DATA1 1 [expr round(rand()*5)]
	force -deposit /counter_tb/iC.DATA0 0 [expr round(rand()*5)]
  } else {
    force -deposit /counter_tb/iC.DATA1 0 [expr round(rand()*5)]
	force -deposit /counter_tb/iC.DATA0 1 [expr round(rand()*5)]
  }
  
  force -deposit /counter_tb/from_next $Control [expr round(rand()*5)]
}

proc null_inputs {Control} {
    force -deposit /counter_tb/iA.DATA1 0 [expr round(rand()*5)]
	force -deposit /counter_tb/iA.DATA0 0 [expr round(rand()*5)]
...
for {set A 0} {$A <= 1} {incr A} {
  for {set B 0} {$B <= 1} {incr B} {
    for {set C 0} {$C <= 1} {incr C} {
      null_inputs 0
	  while {[examine /counter_tb/to_prev] != 1} { run 5 }
	  set_inputs $A $B $C 1
	  while {[examine /counter_tb/to_prev] != 0} { run 5 }
    }
  }
}
